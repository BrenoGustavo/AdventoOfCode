...
def guard_patrol(laboratorio:list, i:int, j:int) -> int: #type: ignore
    ...
    for t in range(len(caminho_a_frente)):
        ...
        if  caminho_a_frente[t] != 'X' and caminho_a_frente[t] !='#' :
            ...
            laboratorio[len(laboratorio) - diferenca - (t+1)][j] = 'X' # Muda elemento para X
            ...
            novo_indice_i = len(laboratorio) -1 - j
            novo_indice_j = len(caminho_a_frente) - (t) 

            return contagem + guard_patrol(laboratorio_girado, novo_indice_i, novo_indice_j)
