...
input_locs = [x for x in open(puzzle_txt).read().split('\n')]
locs = {complex(i,j):input_locs[j][i] for j in range(len(input_locs)) for i in range(len(input_locs[0]))}

deltas = {1,-1,1j,-1j} # legal deltas for area
all_deltas = {1, -1, 1j, -1j,1 + 1j, 1 - 1j, -1 + 1j, -1 - 1j} # deltas for finding perimeter
neighs = {l:[l + d for d in deltas if l + d in locs and locs[l + d] == locs[l] ] for l in locs} # neighbors for area
outside_neighs = {l : set([l + d for d in deltas if l + d not in neighs[l]]) for l in locs} # neighbors for perimeter
...
def get_sides(area):
    ...
    border = set([z + d for z in area for d in all_deltas if z + d not in area]) #get all spaces one layer away from the area itself, including diagonal moves
    ...
    while len(border) > 0: # continue while we haven't traced any separate border location
        ...
        if out_done == False: # do the outside border first
            left_pts = [z for z in border if z.real == min([z.real for z in border])] #find the leftmost points
            down = [z for z in left_pts if z.imag == max([z.imag for z in left_pts])] #find the bottommost points of those
            p = down[0] # starting point
            seen = set([(p,d)])# record the pairs of points, and direction of movement seen
            done = set([p])
            out_done = True
        else: # deal with a remaining inside border region
            right_pts = [z for z in border if z.real == max([z.real for z in border])] # rightmost inside border points left
            down = [z for z in right_pts if z.imag == max([z.imag for z in right_pts])] # bottommost of these
            p = down[0] # starting point
            seen.add((p,d))
            done.add(p)
